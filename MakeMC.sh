...
echo "Upper / Lower indices are "$UPPER_VERTEX_INDICES" and "$LOWER_VERTEX_INDICES" "
    ...
    export RCDB_CONNECTION=sqlite:////group/halld/www/halldweb/html/dist/rcdb.sqlite
		...
		for rootfile in $rootfiles; do
			...
			if [[ "$filecheck" == "0" ]]; then
				...
				gen_test=`echo $filetomv | grep genOut.root`
